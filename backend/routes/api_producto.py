...
@api_producto.route('/productos/por_caducar/<external_id>')
@token_required
def productos_por_caducar(external_id):
    productos_por_caducar_por_lote = censoC.listarporCaducar()
    return make_response(
        jsonify({"msg": "OK", "code": 200, "datos": [i.serialize for i in censoC.listar_productos_por_caducar(external_id)]}),
        200
    )


@api_producto.route("/producto/listar/caducados/<external_id>")
def listar_productos_caducados(external_id):
    productos_caducar_por_lote = censoC.listarCaducados()
    return make_response(
        jsonify({"msg": "OK", "code": 200, "datos": [i.serialize for i in censoC.listar_productos_caducados(external_id)]}),
        200
    )

@api_producto.route('/productos/Sucursal/<external_id>')
@token_required
def listar_productos_Sucursal(external_id):
    return make_response(
        jsonify({"msg": "OK", "code": 200, "datos": [i.serialize for i in censoC.listar_productos_Sucursal(external_id)]}),
        200
    )

@api_producto.route('/productos/buenos/<external_id>')
@token_required
def productos_buenos(external_id):
    return make_response(
        jsonify({"msg": "OK", "code": 200, "datos": [i.serialize for i in censoC.listar_productos_buenos(external_id)]}),
        200
    )
