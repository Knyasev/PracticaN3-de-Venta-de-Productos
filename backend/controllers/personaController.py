...
class PersonaController:
    ...
    def modificar_cuenta(self, data,external_id):
        cuenta = Cuenta.query.filter_by(external_id=external_id).first()
        if cuenta:
            persona = Persona.query.filter_by(id=cuenta.persona_id).first()
            if persona:
                persona.apellido = data.get("apellido", persona.apellido)
                persona.nombre = data.get("nombre", persona.nombre)
                cuenta.usuario = data.get("correo", cuenta.usuario)
                cuenta.clave = generate_password_hash(data.get("clave")) if data.get("clave") else cuenta.clave
                db.session.commit()
                return cuenta.id
            else:
                return -3  # Persona no encontrada
        else:
            return -2  # Cuenta no encontrada
