...
class ProductoController:
    ...
    def listar_productos_Sucursal(self, external_id):
        sucursal = Sucursal.query.filter_by(external_id=external_id).first()
        if sucursal:
            return Producto.query.filter_by( Sucursal_id=sucursal.id).all()
        
    def listar_productos_buenos(self, external_id):
        sucursal = Sucursal.query.filter_by(external_id=external_id).first()
        if sucursal:
            return Producto.query.filter_by(estado="BUENO", Sucursal_id=sucursal.id).all()
        
    def listar_productos_caducados(self,external_id):
        sucursal = Sucursal.query.filter_by(external_id=external_id).first()
        if sucursal:
            return Producto.query.filter_by(estado="CADUCADO",Sucursal_id= sucursal.id).all()
    
    def listar_productos_por_caducar(self,external_id):
        sucursal = Sucursal.query.filter_by(external_id=external_id).first()
        if sucursal:
            return Producto.query.filter_by(estado="POR_CADUCAR",Sucursal_id = sucursal.id).all()
    ...
    def listarporCaducar(self):
        ...
        for lote in lotes:
            ...
            # Contar productos buenos en el lote
            productos_buenos = Producto.query.filter(Producto.lote_id == lote.id, Producto.status == True).count()
    
            for producto in productos_por_caducar:
                if producto.status == True:
                    producto.estado = "POR_CADUCAR"
                    producto.stock = 0  # Set stock to 0
                    producto.status = False
                else:
                    producto.estado = "POR_CADUCAR"
    
            # Ajustar el stock de todos los productos del lote al número de productos buenos
            productos_del_lote = Producto.query.filter(Producto.lote_id == lote.id).all()
            for producto in productos_del_lote:
                producto.stock = productos_buenos
            ...
            for lote_mismo_nombre in lotes_mismo_nombre:
                productos_a_reducir_stock = Producto.query.filter(
                    (Producto.estado == "POR_CADUCAR") | (Producto.id.in_([p.id for p in productos_por_caducar])),
                    Producto.lote_id == lote_mismo_nombre.id
                ).all()
                for producto in productos_a_reducir_stock:
                    if producto.status == True:  # Only reduce stock if status is True
